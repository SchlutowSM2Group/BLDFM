...
.. note::
   ...
   Plotting and I/O functions have moved to
   `abl-tk <https://github.com/SchlutowSM2Group/abl-tk>`_.  Import them from
   ``abltk.plotting`` and ``abltk.io.netcdf``.
...
.. code-block:: python
    ...
    from bldfm import load_config, run_bldfm_single
...
.. code-block:: python
    ...
    from abltk.plotting import plot_footprint_field, plot_footprint_timeseries
    import numpy as np
...
.. code-block:: python
    ...
    from bldfm import load_config, run_bldfm_multitower
    from abltk.plotting import plot_footprint_field
    from abltk.io.netcdf import save_netcdf

    config = load_config("my_config.yaml")
    results = run_bldfm_multitower(config)

    # Save to NetCDF
    save_netcdf(results, config, "output/footprints.nc")
...
Source area analysis and plotting functions are available in ``abltk``.
