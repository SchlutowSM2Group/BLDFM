"""
Error convergence test: numerical vs analytic solutions across grid resolutions.
...
if __name__ == "__main__":
    ...
    # --- Build base config (high-res analytic reference) ---
    initialize()
    ...
    conc_ana = result_ref["conc"]
    flx_ana = result_ref["flx"]
    ...
    for i, (modes, nz) in enumerate(zip(modess, nzs)):
        ...
        conc_err[i] = np.mean((result["conc"] - conc_ana) ** 2) / np.mean(conc_ana**2)
        flx_err[i] = np.mean((result["flx"] - flx_ana) ** 2) / np.mean(flx_ana**2)
