...
class FFTManager:
    ...
    def __init__(self, wisdom_file="fftw_wisdom.pkl", num_threads=1, cache_keepalive=30):
        ...
        logger.info(f"FFTManager initialized with {num_threads} threads, cache keepalive {cache_keepalive}s")
            ...
            if self.wisdom_file.exists():
                with open(self.wisdom_file, 'rb') as f:
                    ...
                    if all(import_results):
                        logger.info(f"Successfully loaded wisdom from {self.wisdom_file}")
                    else:
                        logger.warning("Wisdom import partially failed, will be regenerated")
            else:
                logger.info("No existing wisdom file found, will be generated during first use")
...
def get_fft_manager(num_threads=1, cache_keepalive=30):
    ...
    if _fft_manager is None:
        _fft_manager = FFTManager(num_threads=num_threads, cache_keepalive=cache_keepalive)
    return _fft_manager
